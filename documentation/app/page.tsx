  ...
  return <main className="landing">
    <section className="hero">
      ...
      <div className="hero-content">
        ...
        <p className="hero-copy">A hierarchical heatmap SDK with shared Kotlin layout and color calculations. Compose on Android, iOS, and Desktop; React SVG on the web.</p>
        <div className="hero-actions"><Link className="button primary" href="/en/docs/getting-started">Compose guide <span>→</span></Link><Link className="button secondary" href="/en/docs/react">React guide</Link></div>
        <dl className="hero-stats"><div><dt>4</dt><dd>platform targets</dd></div><div><dt>5K</dt><dd>leaf benchmark</dd></div><div><dt>0</dt><dd>forced image loaders</dd></div></dl>
      </div>
    </section>

    <section className="proof"><p>SHARED CORE, PLATFORM-SPECIFIC UI</p><div><span>Android</span><span>iOS</span><span>Desktop</span><span>React web</span></div></section>
    ...
    <section className="code-section"><div><p className="eyebrow"><span>02</span> COMPOSE ACROSS THREE HOSTS</p><h2>Build once in<br /><em>commonMain.</em></h2><p>Area comes from <code>value</code>. Color comes from <code>metric</code>. For React, install <code>@taewooyo/heatmap-react</code> and render SVG with the same calculation core.</p><Link href="/en/docs/architecture" className="text-link">Explore the architecture →</Link><br /><Link href="/en/docs/react" className="text-link">Build with React →</Link></div><pre><code>{code}</code></pre></section>
